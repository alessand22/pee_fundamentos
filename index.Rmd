...
author:
- affiliation: GEAG e PPGEMAT/IFPA
  email: alessandro.correa@ifpa.edu.br
  name: Prof. Alessandro de Castro Corrêa, D.Sc.
...
htmltools::img(src = knitr::image_uri("Figuras/logo_ppgemat.jpg"),
               # src = knitr::image_uri("Figuras/logo_geag_4.jpg"),
               alt = 'logo', 
...
<p align="center"> ![](figuras\img_pop_amostra.png){ width=60% } </p>
...
**Independentes (Não Pareadas ou Não relacionadas)**

<p align="center"> ![](figuras\img_independentes.png){ width=60% } </p>


**Dependentes (Pareadas ou Relacionadas)**

<p align="center"> ![](figuras\img_dependentes.png){ width=60% }</p>
...
<p align="center">
![](figuras\img_quartis.png){ width=60% }</p>
</p>
...
<p align="center">![](figuras\img_skewness.png){ width=80% }</p>
...
plot(function(x) dnorm(x, 0,.8),-4,4, ylim=c(0,.5),col=3, xaxt="n",yaxt="n",ann=FALSE)
plot(function(x) dnorm(x,  0,1.3),-4,4, add=T,col=2)
plot(function(x) dnorm(x, 0,1),-4,4, add=T,col=1)
#legend(120,0.05,c("N(100,64)","N(90,64)","N(100,225)"),fill=1:3, bty = "n", pch=20,pt.cex=3)
legend(1.8,0.5,c("Leptocúrtica","Mesocúrtica","Plantocúrtica"), bty = "n", pch=20 , col=c(3,1,2), text.col = "black", cex=1.2, pt.cex=3)
title("Curtose")
```

Vamos analisar os dados de experimentos de tempo de vida útil de baterias em anos que estão no documento "bateria.RData". Iniciamos importando os dados e examinado o objeto baterias que é um data frame que possui uma variável denominada vida_util, com 40 observações.
...
stripchart(bateria$vida_util, 
           ...
           at = .1, 
           pch = 19)
title(main = "observações dos tempos de vida útil de baterias em anos", xlab ="anos")
...
```{r, echo=FALSE}
load("bateria.RData") # carregar dados em .RData
grafico <- boxplot(bateria$vida_util,yaxt="n")
boxplot(bateria$vida_util,yaxt="n")
title(main="Boxplot")

text(0.55, grafico$stats[1], "Lim. inferior", cex = 0.9, col="red")
arrows(0.65,grafico$stats[1], 0.78,grafico$stats[1],col="red",length=.1,angle=25)
text(0.55, grafico$stats[2], "1o Quartil", cex = 0.9, col="red")
arrows(0.65,grafico$stats[2], 0.78,grafico$stats[2],col="red",length=.1,angle=25)
text(0.55, grafico$stats[3], "Mediana", cex = 0.9, col="red")
arrows(0.65,grafico$stats[3], 0.78,grafico$stats[3],col="red",length=.1,angle=25)
text(0.55, grafico$stats[4], "3o Quartil", cex = 0.9, col="red")
arrows(0.65,grafico$stats[4], 0.78,grafico$stats[4],col="red",length=.1,angle=25)
text(0.55, grafico$stats[5], "Lim. superior", cex = 0.9, col="red")
arrows(0.65,grafico$stats[5], 0.78,grafico$stats[5],col="red",length=.1,angle=25)
text(0.70, grafico$out[1], "Outlier", cex = 0.9, col="red")
arrows(0.80,grafico$out[1], 0.93,grafico$out[1],col="red",length=.1,angle=25)
text(0.70, grafico$out[2], "Outlier", cex = 0.9, col="red")
arrows(0.80,grafico$out[2], 0.93,grafico$out[2],col="red",length=.1,angle=25)

text(1.28, (grafico$stats[2]+grafico$stats[4])/2, "IQR", cex = 0.9, col="red")
arrows(1.22,grafico$stats[2],1.22,grafico$stats[4],col="red",length=.1,angle=25,code = 3)
```
...
Boxplot(bateria$vida_util,
        ...
        col="yellow", 
        las=1)
title(main="Boxplot com a identificão dos outliers")
points(xbar, pch='+', cex=2, col="red") # média
```
...
### Exercício
Os dados da força de ruptura em libras por polegada quadrada para 100 garrafas descartáveis de 1 litro de refrigerante estão no arquivo **dadosForca.R**. Essas medidas foram obtidas, testando-se cada garrafa até ocorrer a quebra. (FONTE: HINES et al. (2006), Tabela 8-2, p. 157).
...
Para uma tabala de frequência personalizada, os recursos do comando `transform()` são muito convenientes por permitir o cálculo das colunas a partir dos valores da coluna inicial que será justamente a tabela de frequências acumuladas já criada previamente. A tabela resultante é esteticamente atraente e permite uma manipulação posterior fácil por ser um data.frame.
...
hist(dadosForca$forca,
     ...
     col = "chocolate",
     border = "brown",
     main = "Histograma com No e intervalos de classes sugeridos pelo R")
```
...
<p align="center">![](figuras\img_graus.png){ width=50% }</p>
...
<p align="center">
![](figuras\probRoleta1.png){ width=60% }</p>
...
<p align="center">
![](figuras\probRoleta2.png){ width=60% }</p>
...
plot(x, y, main="distribuição (Curva) Normal",xlab="",ylab="",type="l",yaxt="n", xaxt='n', col="blue",lwd=2,frame.plot=FALSE)
...
plot(x, y, main="distribuição de QI",xlab="",ylab="",type="l",yaxt="n",xaxt="n", col="blue",lwd=2,frame.plot=FALSE)
...
title(main="Área do lado direito da curva normal")
axis(1, at=-4:4, labels=c("",paste(-3:-1, "s", sep=""),expression(bar(x)),paste(1:3, "s", sep=""),""))
...
x <- seq(-4, 4, by=0.1) # Serie de x
y <-dnorm(x,mean=0,sd=1) # y como densidade de x
plot(x, y, main="Área total sob a curva normal",xlab="",ylab="",type="l",yaxt="n", xaxt='n', col=1,lwd=1,frame.plot=FALSE)
axis(1, at=-4:4, labels=c("",paste(-3:-1, "s", sep=""),expression(bar(x)),paste(1:3, "s", sep=""),""))
...
```{r, echo=FALSE}
x <- seq(-3,3, length=100)
plot(function(x) dnorm(x, 0,1),-4,4, ylab = "f(x)", xlab="")
plot(function(x) dnorm(x,  0,1.3),-4,4, add=T,col=2)
plot(function(x) dnorm(x, 1,1),-4,4, add=T,col=3)
axis(side = 1, at = c(-4:4))
#legend(120,0.05,c("N(100,64)","N(90,64)","N(100,225)"),fill=1:3, bty = "n", pch=20,pt.cex=3)
legend(2.5,0.35,c("N(0,1)","N(0,1.3)","N(1,1)"), bty = "n", pch=20 , col=1:4, text.col = "black", cex=1.2, pt.cex=3)
title("Várias curvas normais")
```

```{r, echo=FALSE}
x <- seq(-4, 4, by=0.1) # Serie de x
y <-dnorm(x,mean=0,sd=1) # y como densidade de x
plot(x, y, main="Curva Normal Padronizada",xlab="",ylab="",type="l",yaxt="n", xaxt='n', col="blue",lwd=2,frame.plot=FALSE)
...
x <- seq(55, 145) # Serie de x
y <-dnorm(x,mean=100,sd=15) # y como densidade de x
plot(x, y, main="distribuição de QI",xlab="",ylab="",type="l",yaxt="n",xaxt="n", col="blue",lwd=1,frame.plot=FALSE)
axis(side = 1, at = c(seq(55,145, by=15)))
...
```{r, echo=FALSE}
curve(dnorm(x), -4, 4, col = "blue", xlab = "",ylab = "")
curve(dt(x, df = 3), add = TRUE)
title("Comparação entre a curva padrão e distribuição t com 3 gl")
```
...
<p align="center">![](figuras\img_ic_conceito.png){ width=50% }</p>
...
<p align="center">![](figuras\img_ic.png){ width=40% }</p>
...
<p align="center"> ![](figuras\img_erro_tipo.png){ width=40% } </p>
...
y <- data.frame(Modified=y1,Unmodified=y2) # com rotulos

# análise exploratória
# Estisticas descritivas
attach(y)
y.means <- apply(y,2,mean) # média por 2 (coluna)

# análise descritiva gráfica
stripchart(y,xlab=expression(paste("Strength (",kgf/cm^2,")")),pch=19)
...
points(y.means, pch='+', cex=1.5, col=1)
```
...
qqnorm(Modified, main="Modified - Normal Q-Q plot"); qqline(Modified)
qqnorm(Unmodified,main="Unmodified - Normal Q-Q plot"); qqline(Unmodified)

# Valores padronizados
qqnorm(scale(Modified), main="Modified padronizado - Normal Q-Q plot"); qqline(scale(Modified))
qqnorm(scale(Unmodified),main="Unmodified padronizado - Normal Q-Q plot"); qqline(scale(Unmodified))

# Histogramas
hist(scale(Modified), freq=FALSE)
curve(dnorm(x), add=TRUE, col=2); lines(density(scale(Modified)), col=3)
legend(.5,0.6, c("Dens. teórica","Dens. empírica"), fill=c("red", "green"),box.col=0, horiz=F )


hist(scale(Unmodified), freq=FALSE)
curve(dnorm(x), add=TRUE, col=2); lines(density(scale(Unmodified)), col=3)
...
library(car)
# Organizar dados em df. Uma coluna com dados e outra com fator indicado os tratamentos
resp<-c(Modified, Unmodified)
trat<-rep(c("Mod","Unm"),c(10,10))
dados<-data.frame(resposta=resp,tratamento=trat)

leveneTest(resposta~tratamento ,data=dados) # Brown-Forsythe test
leveneTest(resposta~tratamento, center = mean ,data=dados) # Levene
...
t.test(Modified, Unmodified, var.equal=T)
```
